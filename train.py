...
def CTDE_train(
    ...
    ## ================= SAVE TRAINED MODEL =================
    model_dir = os.path.join(training_dir, "results")
    os.makedirs(model_dir, exist_ok=True)

    central_server.save(model_dir)

    print(f"[SAVE] Model saved to {model_dir}")

    print(f"Training finished in {time.time() - start_time:.2f}s")

def evaluate(env, central_server, num_episodes, training_dir=None, random_policy=False):
    rewards, drops = [], []
    delays, energies = [], []
...
def main(args):
    ...
    # --------------------------------------------------
# Ensure env exists for model shape construction
# --------------------------------------------------
    if env is None:
        env = Offload(
        args.num_iot,
        ...
        args.num_time + args.max_delay,
        args.max_delay,
        args.task_arrival_prob,  # OK to use test prob here
    )
    ...
    if args.load_path is not None:
        model_dir = os.path.join(args.load_path, "results")

        print(f"[LOAD] Loading model from {model_dir}")

        if args.hybrid:
            central_server.eval_net.load_state_dict(
                torch.load(os.path.join(model_dir, "eval_net.pt"), map_location="cpu")
            )
            central_server.target_net.load_state_dict(
                torch.load(os.path.join(model_dir, "target_net.pt"), map_location="cpu")
            )
        else:
            central_server.eval_net.load_state_dict(
                torch.load(os.path.join(model_dir, "eval_net.pt"), map_location="cpu")
    ...
    if args.load_path is None:
        ...
        print("[INFO] Skipping training (evaluation-only mode)")

# ---------- TEST ENV (distribution shift) ----------
    # ---------- TEST ENV (distribution shift) ----------

    test_p = (
    args.test_arrival_prob
    if args.test_arrival_prob is not None
    else args.task_arrival_prob
    )
    ...
    if args.load_path is None:
        print("[DEBUG] Train p =", env.task_arrive_prob)

    print("[DEBUG] Test  p =", test_env.task_arrive_prob)



# ---------- EVALUATION ----------
    eval_results = evaluate(test_env, central_server, 50, training_dir)
...
if __name__ == "__main__":
    ...
    type=float,
    default=None,
    help="Arrival probability for evaluation (distribution shift)"
    )
