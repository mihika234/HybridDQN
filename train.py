...
#     drop_series = pd.Series(episode_dropped)

#     rolling_return_var = reward_series.rolling(window).var()
#     rolling_drop_var = drop_series.rolling(window).var()

#     np.save(os.path.join(training_dir, "results/rolling_return_var.npy"),
#             rolling_return_var.to_numpy())

#     np.save(os.path.join(training_dir, "results/rolling_drop_var.npy"),
#             rolling_drop_var.to_numpy())
...
#     parser.add_argument("--task_arrival_prob", type=float, default=0.2)
#     parser.add_argument("--num_time", type=int, default=100)
#     parser.add_argument("--max_delay", type=int, default=15)
...
def CTDE_train(
    ...
    "dropped_tasks": [],
        ...
        central_server.finalize_episode_entropy()
        diagnostics.setdefault("decision_time", []).append(
    ...
    threshold = 0.95 * np.max(smooth)

    conv_ep = np.argmax(smooth >= threshold)

    np.save(
        os.path.join(training_dir, "results", "conv_episode.npy"),
        np.array([conv_ep])
    )
    ...
    np.save(
        os.path.join(training_dir, "results", "peak_memory.npy"),
        np.array([peak_memory])
    )

    np.save(
        os.path.join(training_dir, "results", "cpu_usage.npy"),
        np.array(cpu_usage)
    )
    np.save(
        os.path.join(training_dir, "results", "total_steps.npy"),
        np.array([RL_step])
    )
    
    np.save(
        os.path.join(training_dir, "results/pqc_calls.npy"),
        np.array([central_server.eval_net.pqc_calls])
    )
    avg_forward_time = (
        central_server.eval_net.forward_time /
        max(central_server.eval_net.forward_calls, 1)
    )
    np.save(
        os.path.join(training_dir, "results",
                    "avg_forward_time.npy"),
        np.array([avg_forward_time])
    )
    print(f"Training finished in {time.time() - start_time:.2f}s")
    total_training_time = time.time() - start_time
    np.save(
        os.path.join(training_dir, "results", "training_time.npy"),
        np.array([total_training_time])
    )
    throughput = RL_step / total_training_time
    np.save(
    os.path.join(
        training_dir,
        "results",
...
def main(args):
    ...
    param_count = sum(
        p.numel()
        for p in central_server.eval_net.parameters()
        if p.requires_grad
    )
    buffer = io.BytesIO()
    torch.save(
        central_server.eval_net.state_dict(),
        buffer
    )

    model_size_mb = buffer.getbuffer().nbytes / (1024**2)
    np.save(
        os.path.join(training_dir, "results", "model_size_mb.npy"),
        np.array([model_size_mb])
    )
    np.save(
        os.path.join(training_dir, "results", "param_count.npy"),
    ...
    eval_results = evaluate(env, central_server, 100, training_dir)


    print("Evaluation (100 eps):", eval_results)
