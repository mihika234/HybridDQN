...
#         self.episode_action_counts = None
#         self.action_entropy_store = []
...
#                 action = int(torch.argmax(q[0]))

#         # NEW: count action
#         if self.episode_action_counts is not None:
...
#             self.qubit_count = self.n
#             self.circuit_depth = self.layers
#             self.shots = None   # tensorcircuit statevector simulator
...
if TENSORCIRCUIT_AVAILABLE:
    class QuantumNetwork(nn.Module):
        def __init__(
            ...
            # Classical compression before quantum data re-uploading
            self.feature_proj = nn.Linear(self.state_dim, self.n)
            ...
            self.quantum_layer = tc.interfaces.torch_interface(
                self.K.vmap(self.vqc, vectorized_argnums=0),
                jit=True
            )
            ...
            self.qubit_count = self.n
            self.circuit_depth = 7 * self.layers  # reporting estimate
            self.two_qubit_gates = (
                self.n * self.layers if self.n > 2
                else max(0, (self.n - 1) * self.layers)
            )
            self.shots = 0  # analytic expectation values, no finite-shot sampling

        def vqc(self, state, weights):
            """
            Data re-uploading block repeated each layer:
            - encode data angles
            - apply trainable single-qubit processing
            - entangle qubits
            """
            c = tc.Circuit(self.n)
            ...
            return self.K.stack([
                self.K.real(c.expectation((tc.gates.z(), [q])))
                for q in range(self.n)
            ])
        ...
        def forward(self, s, lstm_s):
            ...
            if isinstance(lstm_s, np.ndarray):
                lstm_s = torch.tensor(lstm_s, dtype=torch.float32, device=self.device)

            if s.dim() == 1:
                s = s.unsqueeze(0)
            if lstm_s.dim() == 2:
                lstm_s = lstm_s.unsqueeze(0)
            ...
            # Learned classical compression -> quantum data angles
            angles = torch.tanh(self.feature_proj(state)) * math.pi

            # Logging: how many PQC executions happened
            self.pqc_calls += angles.shape[0]
            ...
            self.forward_time += time.perf_counter() - t_forward
            self.forward_calls += s.shape[0]
