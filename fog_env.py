...
#     #         return True # DROP
#     #     return False # SURVIVE (for now)
#     #     # return current_delay >= self.max_delay
    
#     ### correct soft drops
#     # def _check_soft_drop(self, delay):

#     #     if delay <= self.max_delay:
#     #         return 0.0

#     #     excess = delay - self.max_delay

#     #     return min(1.0, 0.15 * excess)
    
#     def _check_soft_drop(self, delay):

#         # Failure probability starts rising BEFORE the deadline
#         # and becomes very high near/after it.

#         margin = delay - self.max_delay

#         prob = 1.0 / (1.0 + math.exp(-3.0 * (margin + 1)))
...
#                     # Soft deadline evaluated ONCE at completion
#                     if self._check_soft_drop(current_delay):

#                         self.process_delay[
#                             self.task_on_process_local[i]['time'], i
#                         ] = current_delay
...
#                             'iot': i,
#                             'start_time': self.task_on_process_local[i]['time'],
#                             'reward': r,
#                             'dropped': True,
#                             'delay': current_delay,
#                             'energy': 0.0
...
#                         base_energy = self.coeff * (
#                             ((self.comp_cap_iot[i]/self.duration)*1e9)**2
#                         ) * (self.comp_density[i]*1e9) * \
#                             self.task_on_process_local[i]['size'] * 1e-2

#                         local_load = 1.0 / self.n_iot

#                         energy = self._nonlinear_comp_energy(
#                             local_load,
#                             base_energy
#                         )
...
#                         current_delay = self.time_count - task['time'] + 1
#                         self.task_on_process_fog[i][f] = {
#                             'size': task['size'],
#                             'time': task['time'],
#                             'remain': task['size'],
#                             'energy': task['energy_trans']
#                         }
#                         break
...
#                         # SOFT DEADLINE CHECK ONLY ON COMPLETION
#                         if self._check_soft_drop(delay):

#                             self.process_delay[
#                                 self.task_on_process_fog[i][f]['time'],
#                                 i
#                             ] = delay
...
#                             finished_tasks.append({
#                                 'iot': i,
#                                 'start_time': self.task_on_process_fog[i][f]['time'],
#                                 'reward': r,
#                                 'dropped': True,
#                                 'delay': delay,
#                                 'energy': total_energy
